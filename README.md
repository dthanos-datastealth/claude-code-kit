...
| **3. Install + register the dual-graph MCP** | CLAUDE.md's MANDATORY code-search order requires `graph_continue` as the FIRST call for every code lookup; without it the kit falls back to bash grep which the kit's hard rules forbid | Follow upstream dual-graph install (typically `pip install` into a venv producing a `mcp-graph-server` binary), then register: `claude mcp add dual-graph /absolute/path/to/mcp-graph-server -- --stdio`. See [`docs/prereqs.md`](docs/prereqs.md) section 10 for the full recipe. Verify with `claude mcp list \| grep dual-graph` — expect `✓ Connected`. |
